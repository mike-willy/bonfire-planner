...
export default function Quiz() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const res = await fetch(`${apiUrl}/quiz-result`, {
        method: "POST",
        ...
        <div className="grid gap-5">
          {q.options.map((opt, i) => (
            <MotionButton
              ...
              className={`w-full px-6 py-5 rounded-xl shadow-md border-2 transition flex items-center justify-between ${answers[q.id] === opt.mood
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span
                  className={`text-lg font-semibold ${answers[q.id] === opt.mood ? "text-white" : "text-gray-800"
                    }`}
                >
                  {opt.answer}
                </span>
              ...
              <span
                className={`text-sm font-medium capitalize ${answers[q.id] === opt.mood ? "text-white" : "text-indigo-600"
                  }`}
              >
                {opt.mood} vibe
              </span>
      ...
      <div className="mt-8 flex gap-4 flex-wrap justify-center">
        ...
        {currentQ < quiz.length - 1 ? (
          ...
          <button
            ...
            className={`px-6 py-3 rounded-lg font-medium shadow ${hasMinimumAnswers
